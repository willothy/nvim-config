...
function M.project_root()
  ...
  if not git_dir or #git_dir == 0 then return end

  return vim.fs.dirname(vim.fs.joinpath(unpack(git_dir)))
end
...
function M.find_root(markers)
  ...
  while path ~= last and not M.is_root(path) do
    for _, marker in ipairs(markers) do
      if path:joinpath(marker):exists() then return path end
    end
    last = path
    path = path:parent()
  end
...
function M.list_bufs()
  ...
  for _, buf in ipairs(bufs) do
    ...
    if name ~= "" then table.insert(buf_list, name) end
  end
  return buf_list
end
...
local function do_make_case(s, case)
  ...
  local function camelCase()
    ...
    for i, word in ipairs(words) do
      if i > 1 then words[i] = word:sub(1, 1):upper() .. word:sub(2) end
    end
...
M.make_case = setmetatable({}, {
  ...
  __call = function()
    if not last_case then last_case = vim.fn.input("Enter case: ") end
